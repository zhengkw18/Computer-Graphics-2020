...
{
    return (
        m_elements[0] * m_elements[0] + m_elements[1] * m_elements[1] + m_elements[2] * m_elements[2]);
}
...
Vector3f Vector3f::fabs() const
{
    return Vector3f(std::fabs(m_elements[0]), std::fabs(m_elements[1]), std::fabs(m_elements[2]));
}

Vector3f Vector3f::confined() const
{
    return Vector3f(std::min(m_elements[0], 1.0), std::min(m_elements[1], 1.0), std::min(m_elements[2], 1.0));
}

int Vector3f::maxD() const
{
    if (m_elements[0] > m_elements[1]) {
        if (m_elements[0] > m_elements[2]) {
            return 0;
        } else {
            return 2;
        }
...
void Vector3f::Input(std::stringstream& fin)
{
    fin >> m_elements[0] >> m_elements[1] >> m_elements[2];
}

Vector3f Vector3f::rotate(const Vector3f& axis, double angle) const
{
    Vector3f ret;
    double cost = cos(angle);
    double sint = sin(angle);
    ret[0] += m_elements[0] * (axis[0] * axis[0] + (1 - axis[0] * axis[0]) * cost);
    ret[0] += m_elements[1] * (axis[0] * axis[1] * (1 - cost) - axis[2] * sint);
    ret[0] += m_elements[2] * (axis[0] * axis[2] * (1 - cost) + axis[1] * sint);
    ret[1] += m_elements[0] * (axis[1] * axis[0] * (1 - cost) + axis[2] * sint);
    ret[1] += m_elements[1] * (axis[1] * axis[1] + (1 - axis[1] * axis[1]) * cost);
    ret[1] += m_elements[2] * (axis[1] * axis[2] * (1 - cost) - axis[0] * sint);
    ret[2] += m_elements[0] * (axis[2] * axis[0] * (1 - cost) - axis[1] * sint);
    ret[2] += m_elements[1] * (axis[2] * axis[1] * (1 - cost) + axis[0] * sint);
    ret[2] += m_elements[2] * (axis[2] * axis[2] + (1 - axis[2] * axis[2]) * cost);
    return ret;
}

double Vector3f::distance2(const Vector3f& v1, const Vector3f& v2)
{
    Vector3f delta = v1 - v2;
    return Vector3f::dot(delta, delta);
}

double Vector3f::power() const
{
    return (m_elements[0] + m_elements[1] + m_elements[2]) / 3;
}

double Vector3f::max() const
{
    return std::max(m_elements[0], std::max(m_elements[1], m_elements[2]));
}

Vector3f Vector3f::getVertical() const
{
    Vector3f ret = cross(*this, Vector3f(0, 0, 1));
    if (ret.isZero())
        ret = Vector3f(1, 0, 0);
    else
        ret = ret.normalized();
...
{
    return std::fabs(m_elements[0]) < EPS && std::fabs(m_elements[1]) < EPS && std::fabs(m_elements[2]) < EPS;
}
...
Vector3f Vector3f::exp(const Vector3f& v)
{
    return Vector3f(std::exp(v[0]), std::exp(v[1]), std::exp(v[2]));
}

Vector3f Vector3f::min(const Vector3f& v1, const Vector3f& v2)
{
    return Vector3f(std::min(v1[0], v2[0]), std::min(v1[1], v2[1]), std::min(v1[2], v2[2]));
}

Vector3f Vector3f::max(const Vector3f& v1, const Vector3f& v2)
{
    return Vector3f(std::max(v1[0], v2[0]), std::max(v1[1], v2[1]), std::max(v1[2], v2[2]));
}
